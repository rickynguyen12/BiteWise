...
const IACustomerCheckout = () => {
    ...
    const validateCity = (field) => {
        if (/[^a-zA-Z\s]+/.test(field)) return "City can only contain letters (a-z A-Z) only!";
        else return "";
    };

    const validateState = (field) => {
        if (/[^a-zA-Z\s]+/.test(field)) return "State can only contain letters (a-z A-Z) only!";
        else return "";
    };
    ...
    const validateAddress = (field) => {
        if (/[^a-zA-Z0-9\s]+/.test(field)) return "Address can only contain letters (a-z A-Z) or numbers only!";
        else return "";
    };

    const validateZipCode = (field) => {
        if (/[^0-9-\s]+/.test(field)) return "Zip Code can only contain numbers only!";
        else return "";
    }
